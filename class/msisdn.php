		...
		if(isset($_SERVER['HTTP_MSISDN'])){
			...
			if(substr($msisdn,0,5)==88019){
				...
				if(isset($_GET['statusCode']) && isset($_GET['referenceCode']) && isset($_GET['referenceCode']) && isset($_GET['serverReferenceCode']) && isset($_GET['endUserId'])){
					...
					header("Location: http://localhost/gp/index.php?vmsisdn=".$endUserId);
				}
				else{
					...
					header("Location: http://localhost/gp/class/test_send_data.php?servicekey=12345&transactionDate={$transactionDate}&referenceCode={$referenceCode}");
				}		
		}
		else{
			header("Location: fromweb.php");
		}
	}
}
?>